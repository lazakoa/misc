...
function isPalindrome(n) {
    return reverseNum(n) === String(n);
}

function isLychrel(n, k) {
    num = n;
    for (var i = 0; i < k; i++) {
        num += Number(reverseNum(num));
        if (isPalindrome(num)) {
            return false;
        }
    }
...
// correct, super easy compared to 54 :)
console.log(lo.filter(lo.range(1,10001), (x) => {
    return isLychrel(x, 50);}).length);
